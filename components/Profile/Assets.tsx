...
import { AssetGuide } from "utils/GuidesData";
...
function Assets() {
    ...
    if (categories && categories.length > 0) {
      ...
      if (initialSelection) {
        const initialCategory = categories.find((item) =>
          assets?.some((asset) => asset.category === item.name)
        );
  ...
  }, [categories, assets, initialSelection]);
  ...
  useEffect(() => {
    if (fromCategory === 2) {
      ...
      const cat = categories?.filter((el) => el.name === selected.category)[0]
        ?.assets;
      const ast = assets?.find(
        (el) => el.category == selected.category
      )?.assets;
      setCatAsset(
        cat?.filter((obj1) => !ast?.some((obj2) => obj1.name === obj2.name))
      );
    }
  }, [categories, assets, selected, selectedCategory2]);
  ...
  const onSubmit = async () => {
    ...
    try {
      for (const selectedImage of selectedImages) {
        var result = await Add("/profile/asset", {
  ...
  const onSubmitEdit = async () => {
    ...
    var result = await Update(`/profile/asset`, {
      Assetid: AssetId,
  ...
  const cdeleteAsset = async () => {
    setSLoading(true);
    var result = await Remove("/profile/asset", deleteAsset);
    setSLoading(false);
    clearDeleteAsset();
  ...
  const handleWhatsApp = () => {
    ...
    const message = encodeURIComponent(
      `Hey guys, I have the following digital assets, have a look and feel free to ping if you want to club or share it with me, let's save some money.\n\n*My assets are:*\n${assetsMessage}\n\nJoin this super useful platform https://bit.ly/3ODp0MZ`
    );
  ...
  const message = `Hey guys, I have the following digital assets, have a look and feel free to ping if you want to club or share it with me, let's save some money.\n\n*My assets are:*\n${assetsMessage}\n\nJoin this super useful platform https://bit.ly/3ODp0MZ`;
    ...
    <>
      <div className={styles.heading}>
          ...
          <div className={styles.modal}>
            <div className={styles.modalHeader}>
              <p className={styles.modalheading}>
                Share your existing subscriptions with others
              </p>
        ...
        <AiOutlineWhatsApp
          ...
          className={`${styles.whatsapp_icon} step-6`}
        />
            ...
            <div className={styles.container}>
              <div className={styles.tabs}>
                {categories?.map((item, index) => {
                  if (assets?.some((asset) => asset.category === item.name)) {
                    return (
                      <div
                        key={index}
                ...
                {assets
                  ...
                  ?.assets.map((item) => (
                    <div key={item.id} className={styles.asset}>
                      ...
                      {/* <h5 className={styles.friends}>visibility: Friends</h5> */}
                      <div>
                        {item.description == "" ? (
                          <div
                            ...
                            onClick={() => openEditAsset(item)}
                          >
                            <AiFillPlusCircle size={15} /> Add Description
                          </div>
                        ) : (
            ...
            <div>
              ...
              <div className={styles.catAssets}>
                {dropDownAsset
                  ...
                  : catAsset
                      // ?.filter((v: any) => v.name.includes(asset))
                      ?.map((item: any) => (
                        <div
                          key={item.id}
                          onClick={() => handleAssetImageClick(item)}
